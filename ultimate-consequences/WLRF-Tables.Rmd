...
This page is supplementary material to Carwil Bjork-James's article, **"When does lethal repression fail? Unarmed militancy and backfire in Bolivia, 1982–2019,"** to be published in the Bolivian Studies Journal.
...
This document is divided into two parts. Unless you are attempting to reproduce our results from the dataset, feel free to skip over **Preparatory Code Blocks**, and begin reading at **Calculations and Queries Used in the Article**. Here is [a cleaner version of this page]() without the embedded code blocks.
...
```{r echo=FALSE, warning=FALSE, fig.cap = "", results = 'asis'}
wide_event_table(quadrant1, max_larger = max_deaths_all_events) 

outcome_reactable(quadrant1_outcomes)
...
```{r echo=FALSE, warning=FALSE, fig.cap = "", results = 'asis'}
wide_event_table(quadrant2, max_larger = max_deaths_all_events)
...
```{r echo=FALSE, warning=FALSE, fig.cap = "", results = 'asis'}
wide_event_table(quadrant3, max_larger = max_deaths_all_events)
...
```{r echo=FALSE, warning=FALSE, fig.cap = "", results = 'asis'}
wide_event_table(quadrant4, max_larger = max_deaths_all_events)
...
reactable(repression.events.A_outcomes,
                    theme = nytimes())
```
...
reactable(repression.events.B_outcomes,
                    theme = nytimes())
```
...
reactable(repression.events.C_outcomes,
                    theme = nytimes())
```
...
reactable(repression.events.rightlean_outcomes,
                    theme = nytimes())
...
reactable(repression.events.leftlean_outcomes,
                    theme = nytimes())
