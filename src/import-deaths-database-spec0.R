...
import_deaths_database <- function(filename="Bolivia Deaths Database", 
    ...
    if (specification=="spec1.0"){
      stop("This import script is configured for spec0; please use an importer for spec1.\n")
    }else if(!(specification=="Events"|specification=="spec0")){
      warning("Unknown specification; attempting to import using spec0.\n")
    }
    
    # Import the Entries
    if(numeric_dates) {
      datasheet <- read_sheet(ss=database_file, sheet = sheet_name, 
