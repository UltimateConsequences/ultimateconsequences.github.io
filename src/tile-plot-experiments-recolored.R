...
# defines protest_domain.grouped, protest_domain.colors

def <- de.confirmed # reset filtering

# def$protest_domain <- factor(def$protest_domain)
def$protest_domain <- fct_explicit_na(def$protest_domain, na_level = "Unknown")
def$protest_domain <- fct_relevel(def$protest_domain, protest_domain.grouped)

defd <- def %>% filter(!is.na(protest_domain) & !is.na(year))

p <- ggplot(defd, aes(x=year, y=protest_domain, fill=protest_domain, alpha=0.01)) + 
  geom_tile(color="white", size=.5) +
  stat_bin2d(geom = "text", aes(label = ..count..), breaks=seq(1913.5,2022.5,1)) + 
  scale_x_continuous(breaks=1982:2022, limits=c(1981.5,2023.5)) +
  scale_y_discrete(limits = rev) +
...
p <- ggplot(defd, aes(x=year, y=fct_infreq(protest_domain), fill=fct_infreq(protest_domain))) + 
  ...
  annotate("text", x = 2019.5, y = 1, label = "bold(Interim)",
           parse = TRUE) +
  annotate("rect", xmin = 2020,xmax = 2023.5,ymin = 0, ymax = Inf,
           fill="navyblue", 
           alpha = .15) +
  ...
  scale_x_continuous(breaks=1982:2023, limits=c(1981.5,2023.5)) +
  scale_y_discrete(limits = rev) 
